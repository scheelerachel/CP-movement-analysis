...
function [f4,f5,f6,ABstruct] = AsteroidBelt2(movement_path,movement_time,hand,handedness,visit,name,ABstruct)
    ...
    for j=1:length(movement_path)
        ...
        for i=1:length(smoothx)
            if i~=1
                ...
                velocity(i) = dist(i)/dtime(i); % pixels/sec
            else
                dtime(i) = 0;
                dist(i) = 0;
                velocity(i) = 0;
    ...
    graphname = append(name,' ','Asteroid Belt: Average Acceleration');
    title(graphname)
    ylabel('mm/ms^2')
    hold off
    ...
    legend([s1(1) s2(1)],{'Visit 1','Visit 2'})
    graphname = append(name,' ','Asteroid Belt: Maximum Difference of Acceleration (Range)');
    title(graphname)
    ylabel('mm/ms^2')
    hold off
    ...
    legend([s1(1) s2(1)],{'Visit 1','Visit 2'})
    graphname = append(name,' ','Asteroid Belt: Standard Deviation of Acceleration');
    title(graphname)
    ylabel('mm/ms^2')
    hold off
    ...
    % fill structure
    ABstruct.(acc_L_1) = avg_acc_L_1;
    ABstruct.(acc_R_1) = avg_acc_R_1;
    ABstruct.(acc_L_2) = avg_acc_L_2;
    ABstruct.(acc_R_2) = avg_acc_R_2;

    ABstruct.(st_acc_L_1) = std_acc_L_1;
    ABstruct.(st_acc_R_1) = std_acc_R_1;
    ABstruct.(st_acc_L_2) = std_acc_L_2;
    ABstruct.(st_acc_R_2) = std_acc_R_2;

    ABstruct.(diff_acc_L_1) = diff_L_1;
    ABstruct.(diff_acc_R_1) = diff_R_1;
    ABstruct.(diff_acc_L_2) = diff_L_2;
    ABstruct.(diff_acc_R_2) = diff_R_2;
