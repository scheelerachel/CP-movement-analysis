...
%
% Rachel Scheele
% Last Edited 9/3/2025

function [f9, ABstruct] = AsteroidBelt5(lift_pos,other_pos,hand,handedness,visit,name,ABstruct)
    ...
    for i=1:length(lift_pos)
        ...
        count_lifts = length(lifts);
        count_other = length(other);
    ...
    avg_tap_L_1 = mean(tap_L_1);
    avg_tap_R_1 = mean(tap_R_1);
    avg_tap_L_2 = mean(tap_L_2);
    avg_tap_R_2 = mean(tap_R_2);
    ...
    graphname = append(name,' ','Asteroid Belt: Lifts on Path');
    title(graphname)
    ...
    % fill structure
    ABstruct.(label_L_1) = avg_tap_L_1;
    ABstruct.(label_R_1) = avg_tap_R_1;
    ABstruct.(label_L_2) = avg_tap_L_2;
    ABstruct.(label_R_2) = avg_tap_R_2;
