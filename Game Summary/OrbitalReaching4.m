...
function [f18,ORstruct] = OrbitalReaching4(hand,planet,movement_time,handedness,visit,name,ORstruct)
    ...
    for i=1:length(visit)
        if visit(i) == 1
            if isequal(hand{i},'Left')
                ...
                a = a + 1;
                pp_L_1 = cat(1,pp_L_1,planetplot(i));
            elseif isequal(hand{i},'Right')
                com_R_1(b) = completion_right(b);
                b = b + 1;
                pp_R_1 = cat(1,pp_R_1,planetplot(i));
            else
                fprintf('error 5\n');
            end
        elseif visit(i) == 2
            if isequal(hand{i},'Left')
                ...
                a = a + 1;
                pp_L_2 = cat(1,pp_L_2,planetplot(i));
            elseif isequal(hand{i},'Right')
                com_R_2(b) = completion_right(b);
                b = b + 1;
                pp_R_2 = cat(1,pp_R_2,planetplot(i));
            else
                fprintf('error 5\n');
            end
        else
    ...
    graphname = append(name,' ','Orbital Reaching: Average Completion Time');
    title(graphname)
    ylabel('Time (ms)')
    ...
    % fill structure
    ORstruct.(com_time_L_1) = com_L_1;
    ORstruct.(com_time_R_1) = com_R_1;
    ORstruct.(com_time_L_2) = com_L_2;
    ORstruct.(com_time_R_2) = com_R_2;
