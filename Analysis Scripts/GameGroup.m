...
%% Define Variables
for i=1:length(TD)
    participant = string(append(TD(i).ID,'ABVariables'));

    TD(i).RMSE_non1 = mean(TD(i).Asteroid_Belt.(participant).RMS_nondom_1);
    TD(i).RMSE_dom1 = mean(TD(i).Asteroid_Belt.(participant).RMS_dom_1);
    TD(i).RMSE_non2 = mean(TD(i).Asteroid_Belt.(participant).RMS_nondom_2);
    TD(i).RMSE_dom2 = mean(TD(i).Asteroid_Belt.(participant).RMS_dom_2);

    TD(i).comtime_non1 = mean(TD(i).Asteroid_Belt.(participant).completion_time_nondom_1);
    TD(i).comtime_dom1 = mean(TD(i).Asteroid_Belt.(participant).completion_time_dom_1);
    TD(i).comtime_non2 = mean(TD(i).Asteroid_Belt.(participant).completion_time_nondom_2);
    TD(i).comtime_dom2 = mean(TD(i).Asteroid_Belt.(participant).completion_time_dom_2);
end

for i=1:length(UCP)
    participant = string(append(UCP(i).ID,'ABVariables'));

    UCP(i).RMSE_non1 = mean(UCP(i).Asteroid_Belt.(participant).RMS_more_aff_1);
    UCP(i).RMSE_dom1 = mean(UCP(i).Asteroid_Belt.(participant).RMS_less_aff_1);
    UCP(i).RMSE_non2 = mean(UCP(i).Asteroid_Belt.(participant).RMS_more_aff_2);
    UCP(i).RMSE_dom2 = mean(UCP(i).Asteroid_Belt.(participant).RMS_less_aff_2);

    UCP(i).comtime_non1 = mean(UCP(i).Asteroid_Belt.(participant).completion_time_more_aff_1);
    UCP(i).comtime_dom1 = mean(UCP(i).Asteroid_Belt.(participant).completion_time_less_aff_1);
    UCP(i).comtime_non2 = mean(UCP(i).Asteroid_Belt.(participant).completion_time_more_aff_2);
    UCP(i).comtime_dom2 = mean(UCP(i).Asteroid_Belt.(participant).completion_time_less_aff_2);
end
...
[p_RMSE, tbl_RMSE, stats_RMSE] = anovan(RMSE_data,{g1,g2},'interaction');

% Completion Time
comtime_data = [[TD.comtime_dom1] [TD.comtime_non1] [UCP.comtime_dom1] [UCP.comtime_non1]]';
g1 = [repmat("Pref",40,1);repmat("Nonpref",40,1);repmat("Pref",10,1);repmat("Nonpref",10,1)];
g2 = [repmat("TD",80,1);repmat("UCP",20,1)];
[p_ct, tbl_ct, stats_ct] = anovan(comtime_data,{g1,g2},'interaction');