...
function index = comparedisOR(dis_game,dis_MC,prev,graph_title)
    ...
    while run
        ...
        for i=1:size(dis_MC,2)
            ...
            if isnan(x)
                ...
                close figure 1
                sz = length(time_game_adjusted);
                lval = (sz*0.01)+x(end);
                time_game_adjusted = x(end):0.01:lval-0.01;
                idx = time_game_adjusted(1)/0.01;

                % [~,closest] = min(abs(lags_sorted-idx));
                % best_lag = lags_sorted(closest);
                % best_corr = pks_sorted(closest);
