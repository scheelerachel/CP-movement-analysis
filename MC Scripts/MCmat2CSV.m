...
parsefile = append(participant,'_V1ParsedData');
matfile = append(parsefile,'.mat');
MCPfile = fullfile(folder,matfile);
data = load(MCPfile);

svas = fullfile(folder,participant,'V1');
%exportAllTRC(data.(parsefile), frameRate, svas)
exportAllCSV(data.(parsefile), svas);

%% Run function and save V1
parsefile = append(participant,'_V2ParsedData');
matfile = append(parsefile,'.mat');
MCPfile = fullfile(folder,matfile);
data = load(MCPfile);

svas = fullfile(folder,participant,'V2');
%exportAllTRC(data.(parsefile), frameRate, svas)
exportAllCSV(data.(parsefile), svas);